...
export default function ModalPill({ setOpen }: ModalFormWelcomeProps) {
  ...
  return (
    <div>
      {
        pills.knowledgePills.map(({ id, title, description }: { id: string, title: string, description: string}) => {
          const descriptionSliced = description.slice(0, 30)

          return (
            <>
              <div className="mb-4 w-full flex justify-between items-center gap-3" key={id}>
                <div>
                  <p className="text-gray-700 dark:text-gray-300 font-semibold">{title}</p>
                  <p className="text-gray-500 dark:text-gray-300 text-sm whitespace-nowrap overflow-hidden animate-typing">{descriptionSliced}...</p>
                </div>
